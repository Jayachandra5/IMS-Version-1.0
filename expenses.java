...
public class expenses extends parentform {
    ...
    public void rent(Stage stage) {
        ...
        text = new Label("");
        text.setFont(Font.font("Calibri", fw, fp, 16));
        ...
        /*   tf1.focusedProperty().addListener((arg0, oldValue, newValue) -> {
            if (!newValue) { //when focus lost
                if (tf1.getText() == null || tf1.getText().matches("\\s*")) {

                } else if (!tf1.getText().matches("-?\\d+(\\.\\d+)?")) {
                    text.setStyle("-fx-text-fill:red;");
    ...
    public void tax(Stage stage) {
        ...
        /*    tf2.focusedProperty().addListener((arg0, oldValue, newValue) -> {
            if (!newValue) { // when focus lost
                if (tf2.getText() == null || tf2.getText().matches("\\s*")) {
                    // Do Nothing
                } else {
                    dateFormat.setLenient(false);
                    try {
                        dateFormat.parse(tf2.getText());
                        text.setText("");
    ...
    public void bills(Stage stage) {
        ...
        newDate.getChildren().addAll(l4, tf3);
    ...
    public void spoiled(Stage stage) {
        ...
        getDate.getChildren().addAll(l4, tf3);
    ...
    public static void createNewTable() {
      ...
      String url = Constants.mangeDB;
        ...
        try ( Connection conn = DriverManager.getConnection(url);  Statement stmt = conn.createStatement()) {
            // create a new table
            stmt.execute(sql);
            System.out.println(" exe Table created");
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    }
